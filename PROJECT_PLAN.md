...
阶段 5 的自动化真实联调和人眼验收已经通过：VPS 上 Hermes CLI 已通过 SSH agent probe、配置 readiness、contract probe、链式 turn probe、no-GUI app probe 和真实 Tk GUI probe 验证可调用，VPS 现有 Fish Audio 配置可通过 `FISH_AUDIO_API_KEY` 变量名复用，本机麦克风权限和录音信号已通过带超时保护的 `jks_mic_probe` 验证，OLED 已通过全基础情绪 ACK smoke，桌面 GUI 可见操作记录和外接 OLED 物理视频证据均已采集并人工确认。当前仓库不包含真实 API key，也不提交 `.env`；真实 secret 只通过本机 shell 环境或未跟踪 `.env` 注入。若使用 Hermes API Server，`JKS_AGENT_ENDPOINT` 应指向 `http://127.0.0.1:8642/v1/chat/completions`，`JKS_AGENT_TOKEN` 应使用本机 Hermes `API_SERVER_KEY`，若 Hermes profile 暴露非默认 model 则设置 `JKS_AGENT_MODEL`。若使用 VPS Hermes CLI，配置 `JKS_AGENT_HOST`、`JKS_AGENT_USER`、`JKS_AGENT_SSH_PASSWORD`、`JKS_AGENT_COMMAND` 和 `JKS_AGENT_WORKDIR`。Fish Audio 侧需要设置 `JKS_STT_PROVIDER=fish`、`JKS_TTS_PROVIDER=fish`、`JKS_FISH_API_KEY` 或 `FISH_AUDIO_API_KEY`，`JKS_FISH_TTS_MODEL` 默认 `s2-pro`，可选设置 `JKS_TTS_VOICE` 为 Fish voice/reference id。真实联调命令和安全摘要记录在 `docs/ACCEPTANCE_RESULTS.md`。
